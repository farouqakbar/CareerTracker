...
function updateStatusBadge(app) {
  ...
  const icons = { active: "🟢", pending: "🟡", rejected: "🔴", discontinued: "⚫" };
  ...
  if (editBtn) {
    editBtn.addEventListener("click", async () => {
      ...
      if (stageEditMode) {
        ...
        if (h3El) h3El.textContent = currentApp.program + (currentApp.jobTitle ? " — " + currentApp.jobTitle : "");
...
async function deleteAppFull(app, allApps) {
  ...
  const labels = isID
    ? { company: "Perusahaan", program: "Program", stage: "Tahap", warn: "Tindakan ini tidak dapat dibatalkan." }
    : { company: "Company",    program: "Program",  stage: "Stage", warn: "This action cannot be undone." };
