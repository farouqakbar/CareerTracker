...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (signupBtn) {
    signupBtn.addEventListener("click", async () => {
      ...
      try {
        const privacyAccepted = document.getElementById("privacyCheck")?.checked || false;
        const result = await window.auth.register(username.trim(), password, privacyAccepted);
        if (result && result.success) {
          // Mark this as a new user so onboarding tour auto-starts
          localStorage.setItem("ct_tour_new_user", "1");
